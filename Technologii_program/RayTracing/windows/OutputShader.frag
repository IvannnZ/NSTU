...
vec4 castRay(inout vec3 ro, inout vec3 rd) {
	...
	vec3 n;
	mat2x4 spheres[6];
	spheres[0][0] = vec4(-1.0, 0.0, -0.1, 1.0);
	spheres[0][1] = vec4(1.0, 1.0, 1.0, -2.0);
	spheres[1][0] = vec4(5.0, 0.0, -0.01, 1.0);
	spheres[1][1] = vec4(1.0, 1.0, 1.0, 1);
	spheres[2][0] = vec4(1.0, -2.0, -0.01, 1.0);
	spheres[2][1] = vec4(1.0, 0.0, 0.5, 1.0);
	// spheres[3][0] = vec4(3.5, -1.0, 0.5, 0.5);
	// spheres[3][1] = vec4(1.0, 1.0, 0.5, -2.0);
	...
	mat2x4 boxes[2];
    boxes[0][0] = vec4(2.0, 1.0, -0.001, 1.0);
    boxes[0][1] = vec4(0.9, 0.2, 0.2, 1.0);
    boxes[1][0] = vec4(2.0, 5.0, -0.001, -1.0);
    boxes[1][1] = vec4(0.3, 0.1, 0.1, -0.5);
    for (int i = 0; i < boxes.length(); i++){
        vec3 boxN;
        vec3 boxPos = boxes[i][0].xyz;
